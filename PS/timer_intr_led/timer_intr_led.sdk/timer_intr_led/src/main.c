...
int main()
{
	...
	Gpio_Init();
	Timer_Init();
	Timer_Intr_Init(&Intc, &Timer, TIMER_INTERRUPT_ID);
...
void Timer_Intr_Init(XScuGic *InstancePtr, XScuTimer *Timer, u16 TimerIntrId)
{
	...
	XScuGic_Connect(InstancePtr, TimerIntrId, (Xil_ExceptionHandler)Timer_Intr_Handler, (void *)Timer);
	XScuGic_Enable(InstancePtr, TimerIntrId);
